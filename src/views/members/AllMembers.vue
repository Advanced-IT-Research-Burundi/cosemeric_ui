  ...
  <div class="container-fluid py-4">
    <div class="card border-0 shadow-sm">
      <div class="card-header bg-white py-3">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
          ...
          <div class="d-flex gap-2 w-100 w-md-auto">
             ...
             <button class="btn btn-primary text-nowrap" @click="openModal('create')">
      ...
      <div class="card-body p-0">
        <div class="table-responsive">
          <table class="table table-hover align-middle mb-0">
            <thead class="bg-light">
              <tr>
                <th class="ps-4 py-3 text-secondary text-uppercase text-xs font-weight-bolder opacity-7">Matricule</th>
                 <th class="py-3 text-secondary text-uppercase text-xs font-weight-bolder opacity-7">Nom Complet</th>
                <th class="py-3 text-secondary text-uppercase text-xs font-weight-bolder opacity-7">Contact</th>
                <th class="py-3 text-secondary text-uppercase text-xs font-weight-bolder opacity-7">Catégorie</th>
            ...
            <tbody v-if="membersData.length > 0">
              <tr v-for="member in membersData" :key="member.id">
                <td class="ps-4">
                  <span class="badge bg-light text-dark border">{{ member.matricule }}</span>
                </td>
                <td>
                  <div class="d-flex align-items-center">
                    ...
                    <div>
                      ...
                      <small class="text-xs text-muted">Inscrit le {{ formatDate(member.date_adhesion) }}</small>
                    </div>
                  </div>
                </td>
                <td>
                   <div class="d-flex flex-column">
                      <span class="text-xs mb-1"><i class="bi bi-envelope me-1 text-muted"></i> {{ member.email }}</span>
                      <span class="text-xs"><i class="bi bi-telephone me-1 text-muted"></i> {{ member.telephone }}</span>
                   </div>
                </td>
                <td>
                  <span class="text-sm fw-medium text-dark">{{ member.categorie?.nom || member.categorie_id }}</span>
                </td>
                ...
                <td class="text-end pe-4">
                  <div class="btn-group">
                    <button class="btn btn-sm btn-light text-primary" @click="openModal('view', member)" title="Voir">
                        <i class="bi bi-eye"></i>
                    </button>
                    <button class="btn btn-sm btn-light text-info" @click="openModal('edit', member)" title="Modifier">
                        <i class="bi bi-pencil"></i>
                    </button>
                    <button class="btn btn-sm btn-light text-danger" @click="confirmDelete(member)" title="Supprimer">
       ...
       <!-- Pagination -->
      <div class="card-footer bg-white py-3 border-top-0" v-if="pagination.total > 0">
             ...
             <nav aria-label="Page navigation">
                <ul class="pagination pagination-sm mb-0">
                    <li class="page-item" :class="{ disabled: !pagination.prev_page_url }">
                        <button class="page-link" @click="fetchMembers(pagination.current_page - 1)" :disabled="!pagination.prev_page_url">
                    ...
                    <li class="page-item active">
                     ...
                     <li class="page-item" :class="{ disabled: !pagination.next_page_url }">
                         <button class="page-link" @click="fetchMembers(pagination.current_page + 1)" :disabled="!pagination.next_page_url">
    ...
    <div v-if="showViewModal" class="modal fade show d-block" tabindex="-1" role="dialog" @click.self="closeModal">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body" v-if="selectedMember">
             <div class="text-center mb-4">
                 <div class="avatar-xl bg-primary text-white mx-auto rounded-circle d-flex align-items-center justify-content-center mb-3 display-5">
                    {{ getInitials(selectedMember) }}
                 </div>
                 <h4>{{ selectedMember.full_name }}</h4>
                 <p class="text-muted">{{ selectedMember.matricule }}</p>
                 <span class="badge" :class="getStatusBadge(selectedMember.statut)">{{ formatStatus(selectedMember.statut) }}</span>
             </div>
             <ul class="list-group list-group-flush">
                <li class="list-group-item d-flex justify-content-between px-0">
                 ...
                 <li class="list-group-item d-flex justify-content-between px-0">
                    ...
                    <span class="fw-medium">{{ selectedMember.categorie?.nom || '-' }}</span>
                </li>
                 <li class="list-group-item d-flex justify-content-between px-0">
                    <span class="text-muted">Date d'adhésion</span>
                     <span class="fw-medium">{{ formatDate(selectedMember.date_adhesion) }}</span>
                </li>
             </ul>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" @click="closeModal">Fermer</button>
            <button type="button" class="btn btn-primary" @click="openModal('edit', selectedMember); showViewModal=false">Modifier</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showFormModal" class="modal fade show d-block" tabindex="-1" role="dialog" @click.self="closeModal">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                 <div class="modal-header">
                    <h5 class="modal-title">{{ isEditing ? 'Modifier le membre' : 'Nouveau membre' }}</h5>
                    <button type="button" class="btn-close" @click="closeModal"></button>
                 </div>
                        ...
                        <div class="row g-3">
                             ...
                             <div class="col-md-6">
                              ...
                              <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" v-model="formData.matricule" :disabled="isEditing">
                             </div>
                             <div class="col-md-6">
                                <label class="form-label">Date d'adhésion</label>
                                <input type="date" class="form-control" v-model="formData.date_adhesion">
                             </div>
                              <div class="col-md-6">
                                <label class="form-label">Catégorie</label>
                                <select class="form-select" v-model="formData.categorie_id" required>
     ...
     <div v-if="showDeleteModal" class="modal fade show d-block" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Êtes-vous sûr de vouloir supprimer le membre <strong>{{ selectedMember?.full_name }}</strong> ?</p>
                    <p class="text-danger small mb-0"><i class="bi bi-exclamation-triangle me-1"></i> Cette action est irréversible.</p>
                </div>
...
const fillForm = (member) => {
    Object.assign(formData, {
        ...
        date_adhesion: member.date_adhesion ? member.date_adhesion.slice(0, 10) : ''
    });
};
...
const getStatusBadge = (status) => {
    switch (status) {
        case 'actif': return 'bg-success-subtle text-success border border-success';
        case 'inactif': return 'bg-danger-subtle text-danger border border-danger';
        case 'suspendu': return 'bg-warning-subtle text-warning border border-warning';
