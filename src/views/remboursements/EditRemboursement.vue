      ...
      <div class="card-body">
        <form @submit.prevent="handleSubmit">
              ...
              <div class="alert alert-info d-flex align-items-center" role="alert">
                ...
                <div>
                  ...
                  <template v-if="selectedCredit">
                    &nbsp;pour {{ selectedCredit.membre?.nom + ' ' + selectedCredit.membre?.prenom }} | Montant: {{ selectedCredit.montant_accorde || selectedCredit.montant || 0 }}
                  </template>
                </div>
              </div>
            </div>
          ...
          <div class="d-flex justify-content-end gap-2 mt-4">
            <button type="button" class="btn btn-outline-secondary" :disabled="saving" @click="$router.push('/remboursements')">Annuler</button>
...
const fetchData = async () => {
  try {
    ...
    credits.value = Array.isArray(creditsRes.data) ? creditsRes.data : (creditsRes.data?.data || creditsRes.data || []);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    submitError.value = e?.response?.data?.message || 'Erreur lors de la mise à jour';
