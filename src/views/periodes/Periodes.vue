      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ isEdit ? 'Modifier la période' : 'Nouvelle période' }}</h5>
            ...
            <form @submit.prevent="save">
              <div class="row g-3">
                ...
                <div class="col-md-6" v-if="form.type === 'mensuel'">
                  ...
                  <select class="form-select" v-model.number="form.month" required>
                ...
                <div class="col-md-6" v-if="form.type === 'semestriel'">
                  ...
                  <select class="form-select" v-model.number="form.semester" required>
                ...
                <div class="col-md-6">
                  ...
                  <input type="number" class="form-control" v-model.number="form.annee" min="2000" max="2100" required />
                </div>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-outline-secondary" @click="closeModal" :disabled="saving">Annuler</button>
...
const queryParams = ref({ page: 1, per_page: 15, search: '', sort_field: '', sort_order: 'asc', filters: {} });
...
const columns = [
  ...
  { key: 'date_debut', label: 'Début', sortable: true, formatter: v => v ? new Date(v).toLocaleDateString() : '' },
  { key: 'date_fin', label: 'Fin', sortable: true, formatter: v => v ? new Date(v).toLocaleDateString() : '' },
];

const months = ['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'];
const monthLabel = (n) => (n && n >= 1 && n <= 12) ? months[n-1] : '';
const semesterLabel = (s) => s === 1 ? '1er semestre' : (s === 2 ? '2ème semestre' : '');

const form = ref({ id: null, type: 'mensuel', month: 1, semester: 1, annee: new Date().getFullYear(), statut: 'open', start_date: '', end_date: '' });
...
const fetchPeriodes = async () => {
  loading.value = true;
  try {
    const params = { page: queryParams.value.page, per_page: queryParams.value.per_page };
    if (queryParams.value.search) params.search = queryParams.value.search;
    if (queryParams.value.sort_field) { params.sort_field = queryParams.value.sort_field; params.sort_order = queryParams.value.sort_order; }
    Object.entries(queryParams.value.filters).forEach(([k, v]) => { if (v) params[`filter[${k}]`] = v; });

    const res = await api.get('/periodes', params);
    periodes.value = res.data || [];
    store.state.data.periodes = res.data || [];
    
  } catch (e) {
    console.error('Error fetching periodes:', e);
    periodes.value = [];
    store.state.data.periodes = [];
  } finally {
    loading.value = false;
  }
};
...
const openAdd = () => {
  isEdit.value = false;
  form.value = { id: null, type: 'mensuel', month: 1, semester: 1, year: new Date().getFullYear(), status: 'open', start_date: '', end_date: '' };
  error.value = '';
...
const save = async () => {
  ...
  try {
    ...
    if (payload.type === 'mensuel') { payload.semestre = null; }
    if (payload.type === 'semestriel') { payload.mois = null; }
...
const handleSearch = (term) => { queryParams.value.search = term; queryParams.value.page = 1; fetchPeriodes(); };
const handleSort = ({ field, order }) => { queryParams.value.sort_field = field; queryParams.value.sort_order = order; queryParams.value.page = 1; fetchPeriodes(); };
const handleFilter = (filters) => { queryParams.value.filters = filters; queryParams.value.page = 1; fetchPeriodes(); };
const handlePageChange = (page) => { queryParams.value.page = page; fetchPeriodes(); };
const handlePerPageChange = (per) => { queryParams.value.per_page = per; queryParams.value.page = 1; fetchPeriodes(); };
